...
int open_s2ang(s2ang_t *s2ang, intn access_mode) 
{
	...
	if (s2ang->access_mode == DFACC_READ) {
		...
		for (ib = 0; ib < NANG; ib++) {
			...
			SDsetattr(s2ang->sds_id[ib], "_FillValue", DFNT_CHAR8, strlen(S2_ang_fillval), (VOIDP)S2_ang_fillval);
			SDsetattr(s2ang->sds_id[ib], "scale_factor", DFNT_CHAR8, strlen(S2_ang_scale_factor), (VOIDP)S2_ang_scale_factor);
			SDsetattr(s2ang->sds_id[ib], "add_offset", DFNT_CHAR8, strlen(S2_ang_add_offset), (VOIDP)S2_ang_add_offset);
		}
