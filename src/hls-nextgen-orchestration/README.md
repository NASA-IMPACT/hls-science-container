...
Individual tasks can opt into per-execution metrics collection by setting `instrument = True` on their class. Currently
instrumented tasks are `RunFmask`, `RunFmaskV5`, and `RunLaSRC` in all three workflows, plus `ProcessPathRows` and
`RunNbar` in the tiling workflow.
