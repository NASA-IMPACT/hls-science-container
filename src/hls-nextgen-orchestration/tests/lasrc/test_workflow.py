...
# command on PATH to build the pipeline. The Sentinel-2 rust path still applies
# ESA's quality mask (as the C path does), which validates its commands.
_DOWNLOAD_COMMANDS = ["unzip", "download_landsat"]
_RUST_S2_COMMANDS = _DOWNLOAD_COMMANDS + [
...
def rust_s2_bins(
    ...
    """Binaries the Sentinel-2 rust path validates at construction."""
    return _install(install_mock_binaries, _RUST_S2_COMMANDS)
...
def test_sentinel_rust_pipeline_shape(rust_s2_bins: Path, base_env: None) -> None:
...
def test_sentinel_rust_provides_aerosol_qa(rust_s2_bins: Path, base_env: None) -> None:
...
def test_sentinel_rust_applies_quality_mask_first(
    rust_s2_bins: Path, base_env: None
) -> None:
