...
@dataclass(frozen=True)
class _ConfigSource(DataSource):
    """Creates a working directory the way each workflow's ``EnvSource`` does."""
    ...
    def fetch(self) -> dict[Asset[Any], Any]:
        ...
        (self.working_dir / "output.hdf").touch()
        return {CONFIG: _Config(working_dir=self.working_dir)}
...
class _FailingTask(Task):
    ...
    def run(self, inputs: dict[Asset[Any], Any]) -> dict[Asset[Any], Any]:
        raise TaskFailure("task failed", exit_code=3)
...
def _cleanup_pipeline(
    ...
    builder = PipelineBuilder().add(_ConfigSource("EnvConfig", working_dir))
    if task is not None:
        builder = builder.add(task)
    return builder.build(cleanup_working_dir=cleanup_working_dir)


def test_cleanup_removes_working_dir_on_success(tmp_path: Path) -> None:
    working_dir = tmp_path / "job-1"

    _cleanup_pipeline(working_dir).run()

    assert not working_dir.exists()


def test_cleanup_removes_working_dir_on_task_failure(tmp_path: Path) -> None:
    working_dir = tmp_path / "job-1"

    context = _cleanup_pipeline(working_dir, _FailingTask("Fail")).run()

    assert context.exit_code == 3
    assert not working_dir.exists()


def test_cleanup_removes_working_dir_when_task_raises(tmp_path: Path) -> None:
    working_dir = tmp_path / "job-1"

    with pytest.raises(RuntimeError, match="unexpected"):
        _cleanup_pipeline(working_dir, _ExplodingTask("Boom")).run()

    assert not working_dir.exists()


def test_cleanup_disabled_keeps_working_dir(tmp_path: Path) -> None:
    working_dir = tmp_path / "job-1"

    _cleanup_pipeline(working_dir, cleanup_working_dir=False).run()

    assert (working_dir / "output.hdf").exists()


def test_cleanup_tolerates_already_removed_dir(tmp_path: Path) -> None:
    working_dir = tmp_path / "job-1"

    @dataclass(frozen=True)
    class RemoveWorkingDir(Task):
        requires = (CONFIG,)

        def run(self, inputs: dict[Asset[Any], Any]) -> dict[Asset[Any], Any]:
            shutil.rmtree(inputs[CONFIG].working_dir)
            return {}

    _cleanup_pipeline(working_dir, RemoveWorkingDir("Remove")).run()
