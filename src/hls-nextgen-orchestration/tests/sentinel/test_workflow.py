...
from hls_nextgen_orchestration.sentinel.mapped_tasks import RunFmaskV5
...
def test_sentinel_pipeline_fmask(
    mock_binaries: Path,
    container_setup: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Verify the Sentinel pipeline uses the Fmask v5 task per granule."""
    granule_ids = [GRANULE_ID_1, GRANULE_ID_2]
    monkeypatch.setenv("GRANULE_LIST", ",".join(granule_ids))

    pipeline = construct_pipeline(granule_ids=granule_ids)
