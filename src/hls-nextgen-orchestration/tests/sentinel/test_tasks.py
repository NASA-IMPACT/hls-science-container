...
def test_get_granule_dir(sentinel_config: EnvConfig, mock_binaries: Path) -> None:
    ...
    task = GetGranuleDir(name="get_dir")
    outputs = task.run({SAFE_DIR: safe_dir})

    assert outputs[SAFE_PRODUCT_GRANULE_DIR] == inner
    assert outputs[GRANULE_MTD_TL] == xml
...
def test_check_solar_zenith(
    ...
    bundle = {GRANULE_MTD_TL: Path("MTD_TL.xml")}
    if valid:
        task.run(bundle)
    else:
...
def test_apply_s2_quality_mask(sentinel_config: EnvConfig, mock_binaries: Path) -> None:
    ...
    outputs = task.run({SAFE_PRODUCT_GRANULE_DIR: inner})
...
def test_run_fmask_s2(sentinel_config: EnvConfig, mock_binaries: Path) -> None:
    ...
    task = RunFmask(name="fmask")
    outputs = task.run({SAFE_PRODUCT_GRANULE_DIR: inner, CONFIG: sentinel_config})
...
def test_process_hdf_parts(sentinel_config: EnvConfig, mock_binaries: Path) -> None:
    ...
    outputs = task.run(
        {ESPA_XML: xml, CONFIG: sentinel_config, LASRC_AEROSOL_QA: lasrc_dir}
    )
...
def test_resample(sentinel_config: EnvConfig, mock_binaries: Path) -> None:
    ...
    outputs = task.run({TRIMMED_HDF: input_hdf, CONFIG: sentinel_config})
